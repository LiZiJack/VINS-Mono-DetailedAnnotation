...
void img_callback(const sensor_msgs::ImageConstPtr &img_msg)
{
    ...
    for (int i = 0; i < NUM_OF_CAM; i++)
    {
        ROS_DEBUG("processing camera %d", i);
            //单目
            //readImage()函数读取图像数据进行处理/*img_callback()最最核心的语句出现在这里，也就是readImage()，
            //这个函数实现了特征的处理和光流的追踪，里面基本上调用了feature_tracker.cpp里面的全部函数。在这里首先进行了单目和双目的判断，
            //由于VINS-Mono是单目的，所以后部分关于双目的判断不用看，作者写在这里我觉得是为VINS-Fusion做准备的。在这里，
            //trackerData[i]再次出现了(见3.1)。重要，trackerData[i].readImage读取图像数据；readImage()传了2个参数，当前帧的图像和当前帧的时间戳,以下内容见feature_tracker.cpp，看一下原函数里面是怎么写的*/
